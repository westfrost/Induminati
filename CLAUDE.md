...
**File:** `/home/user/Induminati/Opskæring/index.html` (single self-contained file, ~1640 lines: inline `<style>` + inline `<script>`, plus `xlsx.full.min.js` (SheetJS 0.18.5, CDN) for reading the optional stock-list upload — otherwise no external deps besides Google Fonts). Current version: **v3.6 · 17-09-2026** (footer at line 1634).
...
**Model: three-stage guillotine cutting**, matching the physical process exactly (comment at line 386 makes this explicit):
...
This is applied unconditionally whenever the two conditions hold, with no minimum-width threshold on the kept remnant — a maintainer might be tempted to add one (e.g. "only do this if the remnant is at least N mm wide, otherwise it's not really reusable"), but that was **not** asked for and should not be added without being explicitly requested; per the stated rule, length always outweighs width here, however narrow the result.
...
- **Dorn viability check**: `perDorn = floor((dorn.maxW - dorn.tillaeg) / g.b)`; if the kerf allowance (`dorn.tillaeg`) alone is ≥ the effective max dorn width, `perDorn` naturally comes out < 2 and dorn packing silently disables for that group — but there's also an explicit UI warning for this exact case (around line 996) computed against `min(dorn.maxW, maxRollW)`.
...
- **Merging identical-layout rows can run up to 3 iterations** interleaved with re-placing leftovers (line 611) — if this ever needs a 4th pass for some edge case, it currently silently stops (bounded loop, not a fixed-point iteration to convergence).
- **History backward compatibility**: `entryInputs()` (line 1524) handles at least 3 different historical localStorage shapes (`v3` current `inputs` object, vs. legacy `rollDefs`/`bandRows`/`bands`) — do not remove this shim without checking whether users still have old entries in `localStorage`.
- **Internal consistency self-check** (around line 1011) — a warning fires if `placedBands + lostBands !== totalBands`; this should be treated as a real bug indicator if it ever appears, not dismissed.
- **`maxPerWidth` interacting with leftovers** produces a specific warning suggesting the user raise/remove the limit (around line 1014) — this is a UX hint, not an automatic retry without the constraint.
...
**v3.6 · 17-09-2026** (footer, line 1634: `Opskæringsberegner · v3.6 · 17-09-2026`).
