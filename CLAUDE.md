...
**File:** `/home/user/Induminati/Opskæring/index.html` (single self-contained file, ~1573 lines: inline `<style>` + inline `<script>`, plus `xlsx.full.min.js` (SheetJS 0.18.5, CDN) for reading the optional stock-list upload — otherwise no external deps besides Google Fonts). Current version: **v3.2 · 14-09-2026** (footer at line 1572).
...
#### 3a. Lagerliste-import og automatisk rullevalg (`vaelgRullerFraLager`, line 791)
...
**Input format.** An uploaded `.xlsx` (read with the same `xlsx.full.min.js`/SheetJS already used by `Ordrer/` and `Kapacitet/`, loaded from CDN — the only external dependency this page has beyond Google Fonts) is expected to be a Business Central-style export with columns `Kode`, `Beskrivelse` (optional), `Bredde`, `Længde`, and `Planlagt disponibel balance`. Column matching is by header name (`parseLagerRows`, line 727), not position, and extra BC columns (`Bruttobehov`, `Fastlagt tilgang`, etc.) are ignored — only `Bredde`/`Længde`/`Planlagt disponibel balance` are required; missing any of them aborts the whole import with a named-column error instead of silently proceeding.

**Antal is derived, not read.** BC tracks this item type by area (m²), not piece count, so `Planlagt disponibel balance` is the *total* m² on hand for that variant — not a roll count. The physical roll count is therefore computed as `Math.round(balance / ((bredde/1000) * (laengde/1000)))`, and a row whose result is more than 0.05 away from a whole number (real export data does have some rounding slack) bumps a "rounding" warning count shown in the status line, without blocking the import. A row with non-positive/non-finite width, length, or balance, or that rounds to 0 rolls, is silently skipped and counted.
...
- **Dorn viability check**: `perDorn = floor((dorn.maxW - dorn.tillaeg) / g.b)`; if the kerf allowance (`dorn.tillaeg`) alone is ≥ the effective max dorn width, `perDorn` naturally comes out < 2 and dorn packing silently disables for that group — but there's also an explicit UI warning for this exact case (around line 981) computed against `min(dorn.maxW, maxRollW)`.
...
- **History backward compatibility**: `entryInputs()` (line 1507) handles at least 3 different historical localStorage shapes (`v3` current `inputs` object, vs. legacy `rollDefs`/`bandRows`/`bands`) — do not remove this shim without checking whether users still have old entries in `localStorage`.
- **Internal consistency self-check** (around line 996) — a warning fires if `placedBands + lostBands !== totalBands`; this should be treated as a real bug indicator if it ever appears, not dismissed.
- **`maxPerWidth` interacting with leftovers** produces a specific warning suggesting the user raise/remove the limit (around line 999) — this is a UX hint, not an automatic retry without the constraint.
- **`vaelgRullerFraLager`'s worst case is genuinely slow-ish, by design, not a bug**: an order that turns out structurally impossible against the uploaded stock is caught early by the feasibility short-circuit (§3a) and returns instantly, but an order that's merely *short* on quantity/length (every constraint is individually satisfiable, there just isn't enough total stock) will exhaust the entire candidate list one `planCutting` call at a time before reporting the shortfall — confirmed at ~300ms for a real 974-roll/52-variant export, which is fine for a manual button click but would not be fine if this function were ever called in a tight loop or automatically on every keystroke; keep it button-triggered.
...
**v3.2 · 14-09-2026** (footer, line 1572: `Opskæringsberegner · v3.2 · 14-09-2026`).
